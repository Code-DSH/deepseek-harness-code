...
- **Goal / scope:** ship a resilient Electron host around the official Harness Web runtime, official-format plugins, one official Home, managed Skills/presets/global prompt, immutable Routing Suite, user-confirmed updater, and independent Watchdog. This checkout is `review/install-update-flow` at `e53f4dd` with the BETA3 / `0.1.1-rc.2.code.1` source baseline; it is prepared as a pull request against `main`.
