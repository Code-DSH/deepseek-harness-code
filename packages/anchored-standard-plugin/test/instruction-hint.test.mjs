...
test('hint warnings never include filesystem error details', async () => {
  const listeners = {}
  const warns = []
  const secret = 'workspace-name-must-not-be-logged'
  const ctx = {
    on(event, callback) { listeners[event] = callback },
    get(service) {
  ...
  const original = {
    kind: 'enter',
    get messages() { throw new Error(secret) },
  }
  const result = await listeners['agent/pre-step']({ agent }, async () => original)
