...
const residentNames = ['bash', 'dev_tool_search', 'skill_load', 'skill_search', 'str_replace_editor']
...
test('a durable tool call promotes the resident catalog', async () => {
  const { listeners } = register()
  const tools = [...residentTools, { name: 'edit' }, { name: 'grep' }]
  const result = await assemble(listeners['system-prompt/assemble'], [{ type: 'tool/call', data: { name: 'bash' } }], tools)
  // Promoted: bootstrap pair plus discovery tools; read/edit/grep are not
  // resident and remain one dev_tool_search away.
  assert.deepEqual(result.tools.map((tool) => tool.name).sort(), residentNames)
})

test('a first assistant message promotes the resident catalog (no tool call needed)', async () => {
  const { listeners } = register()
  const tools = [...residentTools, { name: 'write' }]
...
test('sessions derive promotion independently from their own events', async () => {
  ...
  assert.deepEqual(promoted.tools.map((tool) => tool.name).sort(), residentNames)
...
test('promotion is memoized per session id within one process', async () => {
  ...
  assert.deepEqual(first.tools.map((tool) => tool.name).sort(), residentNames)
  ...
  assert.deepEqual(second.tools.map((tool) => tool.name).sort(), residentNames)
})
...
test('promoteOn tool-call requires a tool call, not just a reply', async () => {
  ...
  assert.deepEqual(withCall.tools.map((tool) => tool.name).sort(), residentNames)
})
...
test('a missing bootstrap tool rejects the selected preset request', async () => {
  const { listeners } = register()
  const tools = [{ name: 'str_replace_editor' }, { name: 'edit' }]
  await assert.rejects(
    assemble(listeners['system-prompt/assemble'], [], tools),
    /missing required phase tool.*bash/,
  )
})
...
test('context-filter warnings never include downstream message or error text', async () => {
  const { listeners, warns } = register()
  const secret = 'prompt-body-must-not-be-logged'
  const message = {
    get source() {
  ...
  const result = await listeners['agent/pre-step'](
    { agent: agent([], 'private-warning') },
    async () => decision,
  )

  assert.equal(result, decision)
  assert.equal(warns.length, 1)
  assert.doesNotMatch(warns[0], new RegExp(secret))
})
...
test('post-compaction without compactionTools stays on the bootstrap pair', async () => {
  const { listeners } = register()
  const tools = [...residentTools, { name: 'read' }]
  const events = [
    { type: 'assistant/message', seq: 1, data: {} },
    { type: 'compaction/end', seq: 2 },
  ]
...
test('a compaction resets promotion; a new signal after the boundary re-promotes', async () => {
  ...
  assert.deepEqual(rePromoted.tools.map((tool) => tool.name).sort(), residentNames)
})
