 ...
 *  - A missing phase tool rejects this preset's request instead of exposing
 *    an unbounded catalog. The unchanged Standard preset remains available.
...
export function apply(ctx, config) {
  ...
  const keepTools = (assembled, keep) => {
    const available = new Set(assembled.tools.map((tool) => tool.name))
    const missing = [...keep].filter((toolName) => !available.has(toolName))
    if (missing.length > 0) {
      throw new Error(`${name}: missing required phase tool(s): ${missing.join(', ')}`)
    }
    return {
      ...assembled,
  ...
  ctx.on('system-prompt/assemble', async (_assembly, context, next) => {
    ...
    if (status.promoted) {
      ...
      return keepTools(assembled, keep)
    }
  ...
  ctx.on('agent/pre-step', async ({ agent }, next) => {
    ...
    try {
      ...
      try {
        ctx.logger.warn(`${name}: pre-step context filter failed; keeping injected context`)
      } catch {}
      return decision
    }
  }, { prepend: true })
