...
const SCAFFOLD_TSDOWN = (
  pkgName: string,
): string => `import { fileURLToPath } from 'node:url'
...
export function apply(ctx: AppContext, config: Config): void {
  ...
  const dshHome = process.env.DSH_HOME || join(homedir(), ".dsh");
  const registryFile =
    config.registryFile || join(dshHome, "super-injector", "registry.json");
  const profileNodeModules =
    config.profileNodeModules ||
  ...
  function hasActiveEntry(pkgName: string): boolean {
    for (const entry of ctx.loader.entries()) {
      ...
      if (
        opts.name === pkgName &&
        entry.fiber &&
        FIBER_NAMES[entry.fiber.state] === "active"
      )
        return true;
    }
    return false;
  ...
  function purgeCache(pkgDir: string): void {
    const loadCache = ctx.loader.internal?.loadCache as
      | Map<string, unknown>
      | undefined;
  ...
  function readSelfReloadState(): { at: number } {
    try {
      const raw = JSON.parse(readFileSync(selfReloadStateFile, "utf8")) as {
        at?: number;
      };
  ...
  function writePatch(appendText: string): boolean {
    try {
      ...
      const appendIds = [...appendText.matchAll(/^\s*- id:\s*([^\s#]+)/gm)].map(
        (m) => m[1],
      );
  ...
  function scheduleHeal(selfEntry: any, reason: string): void {
    try {
      ...
      const pkgName =
        selfEntry?.options?.name ?? "@dsh-external/dsh-super-injector";
      ...
      const heal = (): void => {
        ...
        void (async () => {
          try {
              ...
              try {
                ...
                try {
                  for (const entry of rebootCtx.loader.entries()) {
                    ...
                    if (o.group) continue;
                    if (
                      String(o.name).includes("dsh-super-injector") &&
                      entry.fiber &&
                      FIBER_NAMES[entry.fiber.state] === "active"
                    ) {
                      officialAlive = true;
                ...
                if (officialAlive) {
                  logger.info(
                    "[super-injector] 自愈：第 %d 次 touch patch 官方重装配生效（%s）",
                    attempt,
                    pkgName,
                  );
                  ...
                  auditLog(
                    "heal-ok",
                    `第 ${attempt} 次 touch patch 官方重装配成功（${pkgName}，触发=${reason}）`,
                  );
                ...
                const lc = rebootCtx.loader.internal?.loadCache as
                  | Map<string, unknown>
                  | undefined;
                if (lc && typeof lc.delete === "function") {
                  for (const u of [...lc.keys()]) {
                    if (
                      typeof u === "string" &&
                      decodeURIComponent(u).includes(pkgName)
                    ) {
                ...
                logger.info(
                  "[super-injector] 自愈：第 %d 次 fallback loader.create 重新装配完成（%s）",
                  attempt,
                  pkgName,
                );
                opStats.selfHeal.ok += 1;
                ...
                auditLog(
                  "heal-ok",
                  `第 ${attempt} 次 fallback loader.create 重新装配成功（${pkgName}，触发=${reason}）`,
                );
            ...
            auditLog(
              `heal-failed`,
              `第 ${attempt} 次失败: ${String(e)}（触发=${reason}）`,
            );
            if (attempt < 3) globalThis.setTimeout(heal, 4000);
            else
              auditLog(
                "heal-exhausted",
                "3 次均失败（需人工介入：修复产物后 touch profile patch 触发重装配）",
              );
          }
        })();
      };
      ...
      logger.warn(
        "[super-injector] 自愈已排程（4s 后第 1 次：touch patch 官方装配，最多 3 次，触发=%s）",
        reason,
      );
    } catch (e) {
  ...
  async function reloadPackage(
    ...
    if (urls.length === 0) {
      ...
      try {
        ...
        if (name) {
          ...
          candidates.push(
            join(profileNodeModules, ...parts, "lib", "index.js"),
          );
        }
        ...
        if (libPath) {
          ...
          for (const u of loadCache.keys()) {
            if (typeof u === "string" && decodeURIComponent(u).includes(real)) {
              entryUrl = u;
              break;
            }
          }
    ...
    try {
      ...
      if (fresh.block.length > 0) {
        auditLog(
          "reload-blocked-stale",
          `${urlKey}: ${fresh.block.join("; ")}`,
        );
        return (
          "ERROR: 重载前构建产物预检失败（先修复再重载，否则前端必挂）：\n- " +
          fresh.block.join("\n- ") +
      ...
      if (fresh.warn.length > 0) {
        auditLog(
          "reload-stale-artifacts",
          `${urlKey}: ${fresh.warn.join("; ")}`,
        );
        logger.warn(
          "[super-injector] 重载 %s 构建产物可能过期（未阻断）: %s",
          urlKey,
          fresh.warn.join("; "),
        );
      }
    ...
    if (isSelf) {
      ...
      try {
        ...
        const valid =
          probe &&
          (typeof probe === "function" || typeof probe.apply === "function");
        if (!valid) {
        ...
        void (async () => {
          try {
            ...
            const startWith = async (plugin: any): Promise<void> => {
              const fiber = rebootCtx.registry.plugin(
                plugin,
            ...
            try {
              ...
              if (name) {
                const cm = rebootCtx.get("clientModules") as
                  | { processOne?: (id: string) => unknown }
                  | undefined;
            ...
            logger.info(
              "[super-injector] 重启器重建完成（REPLACE 语义，fiber=%s）",
              entry.fiber?.state,
            );
          } catch (error) {
    ...
    if (!oldJobUsable) {
      ...
      try {
        const fresh = ctx.loader.unwrapExports(
          await ctx.loader.import(entryUrlFinal, () => []),
        );
        for (const entry of ctx.loader.entries()) {
          ...
          if (opts?.name && String(opts.name).includes(match)) {
            ...
            if (fiber && typeof fiber === "object") {
              ...
              if (
                ...
                typeof registry.plugin === "function"
              ) {
                registry.delete(fiber);
                const nf = registry.plugin(
                  fresh,
                  entry.options.config ?? {},
                  () => [],
                );
    ...
    if (!runtime) {
      ...
      if (target?.fiber) {
        try {
          if (typeof target.fiber.dispose === "function")
          ...
          const fresh2 = ctx.loader.unwrapExports(
            await ctx.loader.import(entryUrlFinal, () => []),
          );
          const nf2 = ctx.registry.plugin(
            fresh2,
    ...
    let fresh: any;
    try {
      fresh = ctx.loader.unwrapExports(
        await ctx.loader.import(entryUrlFinal, () => []),
      );
    } catch (e) {
      for (const [u, job] of backup) loadCache.set(u, job);
      return (
        "ERROR: import 失败，已回滚缓存（旧代保留）: " +
        (e instanceof Error ? e.stack : String(e))
      );
    }
    ...
    function currentConfigOf(fallback: unknown): unknown {
      try {
        for (const entry of ctx.loader.entries()) {
          ...
          if (
            opts?.name &&
            String(opts.name).includes(match) &&
    ...
    try {
      ...
      for (const oldFiber of fibers) {
        try {
          const fiber = oldFiber.parent.registry.plugin(
            fresh,
            config,
            () => [],
          );
      ...
      try {
        ...
        for (const oldFiber of fibers) {
          const fiber = oldFiber.parent.registry.plugin(
            oldPlugin,
            currentConfigOf(oldFiber._config),
            () => [],
          );
          fiber.entry = oldFiber.entry;
          if (fiber.entry) fiber.entry.fiber = fiber;
    ...
    const activeEntry = [...ctx.loader.entries()].find((en) => {
      ...
      return (
        !o.group &&
        String(o.name).includes(match) &&
        en.fiber &&
        FIBER_NAMES[en.fiber.state] === "active"
      );
    });
    ...
    try {
      ...
      const cmDbg = ctx.get("clientModules") as
        | {
        ...
        | undefined;
      dbg.push(
        `  cm=${cmDbg ? "yes" : "no"} clientPath(short)=${cmDbg?.clientPath ? String(cmDbg.clientPath(match)) : "?"} clientPath(full)=${cmDbg?.clientPath ? String(cmDbg.clientPath(fullName)) : "?"}`,
      );
      if (cmDbg?.table) {
        const keys: string[] = [];
        for (const k of cmDbg.table.keys()) keys.push(String(k));
        dbg.push(
          `  table keys(${keys.length}): ${keys.filter((k) => k.includes("dsh-external")).join(",") || "(none)"}`,
        );
      }
      appendFileSync(
        join(dshHome, "super-injector", "reload-debug.log"),
        dbg.join("\n") + "\n",
      );
    } catch {
      /* 诊断失败不阻塞 */
  ...
  function listPlugins(): string {
    ...
    for (const entry of ctx.loader.entries()) {
      const opts = entry.options;
      if (opts.group) continue;
      const state = entry.fiber
        ? (FIBER_NAMES[entry.fiber.state] ?? `state:${entry.fiber.state}`)
        : "no-fiber";
      const entryUrl = [...ctx.loader.internal!.loadCache.keys()].find(
        (u) => typeof u === "string" && u.includes(opts.id),
      );
      // 注入插件标记 [injected]（与 bundle 装配区分，避免 hash id 难认）
      const injected = injectedNames.has(opts.name) ? " [injected]" : "";
      lines.push(
        `- [${state}] ${opts.id} (${opts.name})${injected}${opts.disabled ? " [disabled]" : ""}${entryUrl ? "\n    entry: " + entryUrl : ""}`,
      );
    }
  ...
  function findEntry(match: string): any {
    ...
    for (const entry of ctx.loader.entries()) {
      const opts = entry.options;
      if (opts.group) continue;
      if (opts.id.includes(match) || opts.name.includes(match))
  ...
  function restoreStaging(): void {
    try {
      ...
      const data = JSON.parse(readFileSync(stagingFile, "utf8")) as Record<
        string,
        {
          description?: string;
          parameters?: unknown;
  ...
  safeRegister(
    defineTool({
      name: "dev_stage_add",
      description:
        '开发侧挂：把测试/开发工具挂"后侧"（不进 tools schema、不污染缓存前缀），经 dev_stage_call 调用测试。execute 为 JS 代码字符串（function(args, ctx){...}），仅限可信代码。转正用 dev_stage_promote，丢弃用 dev_stage_demote。',
      parameters: {
      ...
      async execute(args: any) {
        if (!args.name || !/^[a-zA-Z0-9_-]+$/.test(args.name))
          return "ERROR: name 缺失或含非法字符";
        if (staged.has(args.name))
          return `ERROR: staging 已存在同名工具（${args.name}），先 dev_stage_demote 或换个名`;
        let fn: Function;
        try {
          fn = new Function(
            "args",
            "ctx",
            `return (${args.execute})(args, ctx)`,
          );
        } catch (e) {
  ...
  safeRegister(
    defineTool({
      name: "dev_stage_call",
      description:
        "调用后侧（staging）工具测试：不进 schema、不污染缓存。args 为传给工具的 JSON 参数对象。",
      parameters: {
      ...
      async execute(a: any) {
        ...
        if (!t)
          return `ERROR: staging 无此工具（${a.name}）——dev_stage_list 查看`;
        try {
  ...
  safeRegister(
    defineTool({
      ...
      async execute() {
        ...
        const lines = [...staged.entries()].map(
          ([name, t]) =>
            `- ${name} ${t.promoted ? "[已转正]" : "[后侧]"} : ${t.description.slice(0, 60)}`,
        );
        return lines.join("\n");
      },
    }),
  ...
  safeRegister(
    defineTool({
      ...
      description:
        '转正：把 staging 工具一键挂"前侧"（正式注册进 tools schema，下一次请求缓存刷新一次）。确认工具有效后使用。',
      parameters: {
        name: { type: "string", required: true, description: "staging 工具名" },
      },
      output: {
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "丢弃/撤回：从 staging 移除工具（若已转正则同时从正式工具集注销）。",
      parameters: {
        name: { type: "string", required: true, description: "staging 工具名" },
      },
      output: {
  ...
  function refreshClientRow(name: string): void {
    try {
      ...
      if (
        changed &&
        typeof cm.compose === "function" &&
        typeof cm.notifyGraphChanged === "function"
      ) {
  ...
  function notifyClientRebuilt(name: string): void {
    try {
      ...
      if (cm && typeof cm.rebuilt === "function") {
        ...
        if (rev)
          logger.info("[super-injector] client bundle 已联动（rev=%s）", rev);
      }
    } catch {
      /* client 联动失败不阻塞 */
    }
  ...
  function loadStats(): OpStatsMap {
    try {
      ...
      const fails = Array.isArray(raw?.lastFailures)
        ? raw.lastFailures.slice(-5)
        : [];
      return {
  ...
  function auditSummary(): string {
    try {
      ...
      const lines = readFileSync(selfHealLogFile, "utf8")
        .trim()
        .split("\n")
        .filter(Boolean);
  ...
  function clientStatus(name: string): string {
    ...
    try {
      ...
      return path
        ? `client ✓ (${path.split(/[\\/]/).slice(-2).join("/")})`
        : `client ✗（已声明 client 但注册失败——检查 client bundle 是否构建（build:client）与 entry 状态）`;
    } catch {
      return "client 状态未知";
    }
  }
  ...
  function recordOp(kind: OpKind, ok: boolean, reason?: string): void {
    ...
    if (ok) bucket.ok += 1;
      ...
      bucket.fail += 1;
      opStats.lastFailures = [
        ...opStats.lastFailures.slice(-4),
        { kind, at: new Date().toISOString(), reason: reason ?? "" },
      ];
    }
    saveStats();
  }
  ...
  function arbitrateOfficial(): void {
    try {
      const entries = [...ctx.loader.entries()].filter(
        (e) =>
          !e.options.group &&
          String(e.options.name).includes("dsh-super-injector"),
      );
      if (entries.length < 2) return;
      const official = entries.find((e) => String(e.id).includes("include:"));
      const ghosts = entries.filter(
        (e) =>
          e !== official && e.fiber && FIBER_NAMES[e.fiber.state] === "active",
      );
      if (!official || !official.disabled || ghosts.length === 0) return;
      auditLog(
        "arbitrate",
        `幽灵压制官方（official=${official.id} disabled，ghosts=${ghosts.map((g) => g.id).join(",")}），排程清理`,
      );
      ...
      globalThis.setTimeout(() => {
        void (async () => {
          try {
            ...
            if (
              ghostRef.fiber &&
              typeof ghostRef.fiber.dispose === "function"
            ) {
            ...
            try {
              if (officialRef.options.disabled !== undefined)
                delete officialRef.options.disabled;
              const parent = officialRef.parent;
              if (parent && Array.isArray(parent.data)) {
                for (const d of parent.data) {
                  if (d && d.id === officialRef.id && d.disabled !== undefined)
                    delete d.disabled;
                }
              }
              officialRef.fiber = undefined;
              if (typeof officialRef.refresh === "function")
                await officialRef.refresh();
              auditLog(
                "arbitrate-ok",
                `官方 entry 恢复（${officialRef.id}），幽灵 ${ghostRef.id} 已清理`,
              );
            } catch (e) {
  ...
  async function inject(dir: string): Promise<string> {
    ...
    if (!existsSync(absDir) || !statSync(absDir).isDirectory())
      return `ERROR: 目录不存在或不可访问: ${absDir}`;
    if (!existsSync(pkgPath))
      return `ERROR: ${absDir} 存在但没有 package.json（不是插件包目录）`;
    let pkg: { name?: string };
    try {
  ...
  async function uninject(match: string): Promise<string> {
    if (match.includes("super-injector"))
      return "ERROR: 拒绝卸载 dsh-super-injector 自身（引导器不可卸载）";
    const steps: string[] = [];
    ...
    if (!steps.some((s) => s.startsWith("entry 已卸载")))
      steps.push("（无匹配 entry）");
    ...
    if (fullName) {
      ...
      if (idShort) {
        ...
        if (already) {
          ...
          steps.push(
            "profile patch 已写 disabled（阻断自装配，防 refresh 加回；writePatch 兼容 [] 初始形式）",
          );
        } else {
    ...
    const hit = reg.find(
      (e) => e.name.includes(match) || e.dir.includes(match),
    );
    if (hit) fullName ??= hit.name;
    const after = reg.filter(
      (e) => !e.name.includes(match) && !e.dir.includes(match),
    );
  ...
  const REGISTER_NAME = new RegExp(
    `register\\(\\{[\\s\\S]*?name:\\s*['"](${SLOT_ALT})['"]`,
  );
  ...
  function clientSkeletonProblems(base: string): string[] {
    ...
    try {
      ...
      if (existsSync(libClient)) {
        ...
        if (
          !/inject\s*=\s*\[[^\]]*['"]slots['"]/.test(lib) &&
          !/inject\s*:\s*\[[^\]]*['"]slots['"]/.test(lib)
        ) {
          problems.push(
            "lib/client.js 缺 inject 含 slots（apply 用 ctx.slots 必须声明——cordis 服务注入契约）",
          );
        }
        if (!REGISTER_NAME.test(lib)) {
          problems.push(
            `lib/client.js 的 register 缺合法 name（应为已知 slot：${KNOWN_SLOTS.join(" / ")}）`,
          );
        }
      }
      // 2. 源码骨架（有 src 时）
      ...
      if (existsSync(clientSrcPath)) {
        ...
        if (!/export const inject\s*=\s*\[[^\]]*['"]slots['"]/.test(src)) {
          problems.push(
            "src/client/index.ts 缺 export const inject = ['slots']（apply 用 ctx.slots 必须声明，否则报 cannot get property 'slots' without inject）",
          );
        }
        if (!REGISTER_NAME.test(src)) {
          problems.push(
            `slots.register 缺合法 name（应为已知 slot：${KNOWN_SLOTS.join(" / ")}——缺了报 slot undefined is not declared）`,
          );
        }
      }
    } catch {
   ...
   */
  function buildFreshnessProblems(base: string): {
    block: string[];
    warn: string[];
  } {
    const block: string[] = [];
    ...
    try {
      ...
      if (
        ...
        block.push(
          "package.json 带 UTF-8 BOM（EF BB BF）——tsdown/JSON.parse 无法解析，构建必挂；用无 BOM 工具重写（node 写文件，勿用 PowerShell Set-Content -Encoding UTF8）",
        );
      }
      const pkg = JSON.parse(readFileSync(pkgPath, "utf8")) as Record<
        ...
        unknown
      >;
      const hasClient = !!(pkg.dsh as Record<string, unknown> | undefined)
        ?.client;
      ...
      if (existsSync(libIndex)) {
        ...
        if (hostLatest > 0 && hostLatest - mt > 8000) {
          warn.push(
            "lib/index.js 过期（src 修改晚于构建 8s+，疑似漏 npm run build——重载会跑旧代码）",
          );
        }
      }
      // client 产物
      if (hasClient) {
        ...
        if (!existsSync(libClient)) {
          block.push(
            "lib/client.js 不存在（package.json 声明了 dsh.client 但没构建 client——先 npm run build:client，否则前端必挂）",
          );
        } else {
          try {
            ...
            if (!content.includes("__ModuleLoader__")) {
              block.push(
                "lib/client.js 不是 tsdown bundle（缺 __ModuleLoader__ 特征——可能被 tsc 覆盖或手改，重新 npm run build:client）",
              );
            }
            let mt = 0;
            try {
            ...
            if (clientLatest > 0 && clientLatest - mt > 8000) {
              warn.push(
                "lib/client.js 过期（src/client 修改晚于构建 8s+，疑似漏 npm run build:client——前端会加载旧 UI）",
              );
            }
          } catch {
            /* 读不到跳过 */
  ...
  function healProfileLinks(): string[] {
    ...
    try {
      ...
      const linkNames = Object.keys(deps).filter((n) =>
        String(deps[n] ?? "").startsWith("link:"),
      );
      for (const name of linkNames) {
        ...
        const linkPath = join(
          linkDir,
          scope ? (name.split("/")[1] as string) : name,
        );
        if (!isHealthyLink(linkPath)) {
          ...
          try {
            ...
            logger.warn(
              "[super-injector] 断电自愈：重建 junction %s → %s",
              name,
              target,
            );
          } catch (err) {
            logger.warn(
              "[super-injector] junction 重建失败 %s: %s",
              name,
              String(err),
            );
          }
      ...
      logger.warn(
        "[super-injector] link 依赖 junction 自愈扫描失败: %s",
        String(err),
      );
    }
    return healed;
  ...
  async function restore(): Promise<void> {
    // ① profile link 依赖 junction 自愈（断电/强制关机后 junction 悬空 → 重建；
    ...
    for (const e of readRegistry()) {
      try {
        ...
        if (block.length > 0) {
          auditLog(
            "restore-skip-bad-client",
            `${e.name} client 骨架/构建产物问题，跳过恢复: ${block.join("; ")}`,
          );
          logger.warn(
            "[super-injector] 恢复 %s 跳过（client 骨架/构建产物问题）: %s",
            e.name,
            block.join("; "),
          );
          continue;
  ...
  ctx.setInterval(() => {
    ...
    for (const e of readRegistry()) {
      if (!watchList.some((w) => w.dir === e.dir))
        watchList.push({ dir: e.dir, match: e.name });
    }
    for (const w of watchList) {
      ...
      if (fp === null) {
        ...
        if (prevDang === undefined || now - prevDang > 30000) {
          ...
          auditLog(
            "watch-dangling",
            `目录不可读，跳过自动重载（源被删/悬空 junction）: ${w.dir}`,
          );
        }
        continue;
      }
      ...
      if (prev !== undefined && prev !== fp) {
        ...
        const precheck = existsSync(libFile)
          ? ctx.loader
              .import(pathToFileURL(libFile).href, () => [])
              .then((ns: unknown) => {
                const probe = ctx.loader.unwrapExports(ns);
                return !!(
                  probe &&
                  (typeof probe === "function" ||
                    typeof probe.apply === "function")
                );
              })
              .catch(() => false)
        ...
        precheck.then((probeOk: boolean) => {
          if (!probeOk) {
            reloading = false;
            auditLog(
              "watch-precheck-blocked",
              `自动重载预检失败（新代码不可加载），保持旧代码: ${w.dir}`,
            );
            logger.warn(
              "[super-injector] 自动重载预检失败，跳过（旧代码继续运行）: %s",
              w.dir,
            );
            return;
          }
    ...
    try {
      ...
      if (table && typeof table.keys === "function") {
        for (const name of table.keys()) {
          if (typeof name === "string" && name.startsWith("dev_"))
            names.push(name);
        }
      }
      for (const name of names) {
        try {
          if (typeof toolsSvc.unregister === "function")
      ...
      if (names.length > 0) {
        logger.warn(
          "[super-injector] 已清理 %d 个僵尸工具残留: %s",
          names.length,
          names.join(","),
        );
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "超级模组注入器：运行时注入任意本地 DSH 插件包（junction 链接 + loader.create，不碰 patch/package.json、不重启）。参数 = 插件包目录绝对路径（含 package.json 与 lib/）",
      parameters: {
        dir: {
          type: "string",
          required: true,
      ...
      async execute(args: { dir: string }) {
        ...
        if (block.length > 0) {
          return (
            "ERROR: 注入前校验发现 client 骨架/构建产物问题（已阻断——缺 inject 的 client 注入后 Tab 必挂，缺 client bundle 前端必挂）：\n- " +
            block.join("\n- ") +
        ...
        if (fresh.warn.length > 0) {
          auditLog(
            "inject-stale-artifacts",
            `${dir}: ${fresh.warn.join("; ")}`,
          );
          logger.warn(
            "[super-injector] 注入 %s 构建产物可能过期（未阻断）: %s",
            dir,
            fresh.warn.join("; "),
          );
        }
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "超级模组卸载器：卸载已注入的插件包——卸 loader entry（fiber dispose，工具/监听全清理）→ 清注入清单 → 删 profile junction → 另写 profile patch disabled 条目（防 include.refresh 加回），免重启。参数 = 包名子串（如 dsh-toy-supermod）",
      parameters: {
        match: {
          type: "string",
          required: true,
          description:
            "包名/路径子串（如 dsh-toy-supermod 或 @dsh-external/dsh-toy-supermod）",
        },
      },
      output: {
        schema: { type: "string" },
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "清 webserver 路由表残留：删除 path 前缀匹配的 exact/prefixes/upgrades 条目（插件热重载残留路由的自愈工具，无需重启）",
      parameters: {
        prefix: {
          type: "string",
      ...
      async execute(args: { prefix: string }) {
        ...
        for (const tableName of ["exact", "prefixes", "upgrades"] as const) {
          ...
          const keys = [...table.keys()].filter((k: unknown) =>
            String(k).startsWith(args.prefix),
          );
          for (const k of keys) {
            table.delete(k);
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "确定性热重载已加载的 bundle 插件包（清缓存 → 重新 import → registry 重建 fiber，失败回滚保留旧代）。不带参数时返回当下已装配插件清单；带参数重载并给出重载前后 fiber 状态对比。",
      parameters: {
        packageName: {
          type: "string",
      ...
      async execute(args: { packageName?: string }) {
        ...
        const after = freshEntry
          ? await waitFiberStable(freshEntry)
          : "（未找到）";
        return (
          result +
          "\n--- 重载前后状态 ---\nbefore: [" +
  ...
  safeRegister(
    defineTool({
      name: "dev_heal_links",
      description:
        "profile link: 依赖 junction 自愈（手动触发，免重启）：扫描 profile package.json 全部 link: 依赖（bundles 装配依赖 + agent preset 解析依赖），悬空/缺失的 node_modules junction 重建。返回重建清单。",
      parameters: {},
      output: {
        schema: { type: "string" },
        render: (_args: unknown, value: unknown) => [
  ...
  safeRegister(
    defineTool({
      name: "dev_fix_patch",
      description:
        'profile patch 修复：扫描 ~/.dsh/profiles/*/cordis.patch.yml，按 entry id 去重（同 id 保留最后一条，备份原文件）——修复 "duplicate loader entry id" 启动崩溃（手动 patch 两次/重复安装造成）。--check 只查不写',
      parameters: {
        profile: {
          type: "string",
      ...
      async execute(args: { profile?: string; check?: boolean }) {
        ...
        try {
          profileDirs = readdirSync(profilesRoot).filter(
            (d) => !d.startsWith("."),
          );
        } catch {
        ...
        if (args.profile)
          profileDirs = profileDirs.filter((d) => d === args.profile);
        if (!profileDirs.length)
          return "ERROR: 未找到 profile: " + (args.profile ?? "（空）");
        const out: string[] = [];
        let fixedAny = false;
        for (const profile of profileDirs) {
          ...
          for (const rec of dup)
            out.push(
              `[${profile}] 修复：id "${rec.id}" 重复，删除 ${rec.count} 条（保留最后一条）`,
            );
          ...
          try {
            ...
            out.push(
              `[${profile}] 写入失败: ${String(e instanceof Error ? e.message : e)}`,
            );
          }
        }
        out.push(
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "列出当下已装配插件清单（id/name/fiber 状态/入口 URL），不执行重载",
      parameters: {},
      output: {
        schema: { type: "string" },
        render: (_args: unknown, value: unknown) => [
      ...
      async execute() {
        ...
        const summary = kinds
          .map((k) => `${k} ${opStats[k].ok}✓/${opStats[k].fail}✗`)
          .join(" | ");
        const failLines = opStats.lastFailures.length
          ? "\n===== 最近失败（可审计）=====\n" +
            opStats.lastFailures
              .map((f) => `[${f.kind} ${f.at}] ${f.reason || "（无原因）"}`)
              .join("\n")
          : "";
        return (
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "热装配一个本地 bundle 插件：改 profile package.json（dependencies 加 link + bundles 数组加包名）→ 建 node_modules junction → loader.create 动态加载（免重启生效）。幂等：已存在的项自动跳过。重启后由 bundles 列表正常装配（双路径一致）",
      parameters: {
        dir: {
          type: "string",
          required: true,
          description:
            "插件包目录绝对路径（须含 package.json，且已 build 出 lib/）",
        },
        profile: {
          type: "string",
      ...
      async execute(args) {
        return withOpLock(async () => {
          ...
          try {
            ...
            const scope = name.startsWith("@") ? name.split("/")[0] : null;
            const linkDir = scope
              ? join(profileDir, "node_modules", scope)
            ...
            if (existsSync(linkPath))
              steps.push("node_modules link 已存在（跳过）");
            else {
              mkdirSync(linkDir, { recursive: true });
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "插件生产线：生成四种形态的插件骨架（toolkit 工具包 / daemon-loop 守护循环(timer+LLM 自主 agent loop) / ui-panel UI 面板 / hybrid 混合）——package.json（peerDeps 范围声明不硬编码版本）+ tsconfig + build.sh（DSH_CHECKOUT 自动探测）+ 形态源码（资源挂 ctx.effect 规范）+ 可选 client 骨架。生成后：dev_build_plugin 构建 → dev_inject_plugin 注入。",
      parameters: {
        dir: {
          type: "string",
          required: true,
        ...
        form: {
          type: "string",
          description:
            "形态：toolkit / daemon-loop / ui-panel / hybrid（缺省 toolkit）",
        },
      ...
      async execute(args: {
        ...
        const pkgName = rawName.startsWith("@")
          ? rawName
          : "@dsh-external/" + rawName;
        try {
          mkdirSync(targetDir, { recursive: true });
          const files: Array<[string, string]> = [
            [
              "package.json",
              scaffoldPackageJson(pkgName, args.description ?? "", form),
            ],
          ...
          if (form === "toolkit") {
            files.push([
              "src/index.ts",
              scaffoldToolkitSrc(pkgName, args.description ?? ""),
            ]);
          } else if (form === "daemon-loop") {
            files.push([
              "src/index.ts",
              scaffoldDaemonSrc(pkgName, args.description ?? ""),
            ]);
          } else if (form === "ui-panel") {
            files.push([
              "src/index.ts",
              scaffoldUiSrc(pkgName, args.description ?? ""),
            ]);
          ...
          if (form === "ui-panel" || form === "hybrid") {
            const clientSrc = readFileSync(
              join(targetDir, "src/client/index.ts"),
              "utf8",
            );
            const problems: string[] = [];
            if (!/export const inject\s*=\s*\[[^\]]*'slots'/.test(clientSrc)) {
              problems.push(
                "缺 export const inject = ['slots']（cordis 服务注入声明——apply 用 ctx.slots 必须声明）",
              );
            }
            if (
              !/register\(\{[\s\S]*?name:\s*['"](?:conversation\.view|settings\.plugin\.item|settings\.plugins\.tab|settings\.section|settings\.general\.item)['"]/.test(
                clientSrc,
              )
            ) {
              problems.push(
                "slots.register 缺合法 name（应为已知 slot：conversation.view / settings.plugin.item / settings.plugins.tab / settings.section / settings.general.item——缺了报 slot undefined is not declared）",
              );
            }
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "插件生产线：构建并打包插件——探测 DSH_CHECKOUT（env/常见路径）→ bash scripts/build.sh（junction link + tsc 编译 host）→ 有 build:client 则 npm run build:client（tsdown 编译 UI）→ npm pack 产出 tgz。依赖：bash/node/npm 在 PATH。返回产物路径。",
      parameters: {
        dir: {
          type: "string",
          required: true,
      ...
      async execute(args: { dir: string }) {
        ...
        if (!pluginDir || !existsSync(join(pluginDir, "package.json")))
          return "ERROR: dir 无 package.json";
        const checkout = detectCheckout();
        if (!checkout)
          return "ERROR: 未找到 DSH checkout（设置 DSH_CHECKOUT 环境变量）";
        ...
        if (typeof scripts["build:client"] === "string") {
          ...
          if (r.code !== 0)
            return `ERROR: build:client 失败（exit ${r.code}）:\n${r.output.slice(-1500)}`;
        ...
        if (r === null) return "ERROR: npm 不可用（需在 PATH）";
        if (r.code !== 0)
          return `ERROR: npm pack 失败（exit ${r.code}）:\n${r.output.slice(-800)}`;
        const tgzMatch = /([^\s]+\.tgz)\s*$/m.exec(r.output.trim());
        if (!tgzMatch)
          return "ERROR: 未识别到 tgz 产物:\n" + r.output.slice(-800);
  ...
  safeRegister(
    defineTool({
      ...
      description:
        "插件生产线：发布 GitHub Release——gh release create v<version> <tgz>（tag + 附件 + notes 模板）。依赖：gh CLI 已认证 + git remote 指向目标仓库。返回 release URL。",
      parameters: {
        dir: {
          type: "string",
          required: true,
      ...
      async execute(args: { dir: string; version?: string; notes?: string }) {
        ...
        if (tgzs.length === 0)
          return "ERROR: 目录无 tgz（先 dev_build_plugin）";
        ...
        if (r === null)
          return (
            "ERROR: gh CLI 不可用（需在 PATH 且已认证）——手动：\ngh release create " +
            tag +
            " " +
            tgz +
            " --repo " +
        ...
        if (r.code !== 0)
          return `ERROR: gh release 失败（exit ${r.code}）:\n${r.output.slice(-800)}`;
  ...
  safeRegister(
    defineTool({
      name: "dev_self_test",
      description:
        "故障演练：一键回归注入器全链路——①注入假插件→host ✓ ②热重载→fiber uid 变化 ③自重载节流拒绝（<10s）④预检拦截（临时破坏 lib→拒绝自杀→恢复）⑤卸载即净 ⑥patch 写入合法性。全部自恢复、不留下污染；输出 PASS/FAIL 清单。",
      parameters: {},
      output: {
        schema: { type: "string" },
        render: (_args: unknown, value: unknown) => [
        ...
        try {
          ...
          if (inj.includes("host ✓")) {
            ...
            try {
              diag +=
                "junction→" +
                readlinkSync(
                  join(profileNodeModules, "@dsh-external", TEST_SHORT),
                ) +
                "\n";
            } catch (e) {
              diag += "junction err: " + (e as { code?: string }).code + "\n";
            }
            diag +=
              "tmpLib=" +
              existsSync(join(tmpDir, "lib", "index.js")) +
              " tmpPkg=" +
              existsSync(join(tmpDir, "package.json")) +
              " tmpSrc=" +
              existsSync(join(tmpDir, "src", "index.ts"));
          ...
          const beforeUid = (() => {
            for (const e of ctx.loader.entries()) {
              const o = e.options;
              if (!o.group && String(o.name) === TEST_PKG && e.fiber)
                return e.fiber.uid;
            }
            return null;
          })();
          try {
            ...
            const lcT = ctx.loader.internal?.loadCache as
              | Map<string, unknown>
              | undefined;
            if (lcT) {
              for (const u of [...lcT.keys()]) {
                if (
                  typeof u === "string" &&
                  decodeURIComponent(u).includes(TEST_SHORT)
                )
                  Map.prototype.delete.call(lcT, u);
              }
            }
          ...
          check(
            "热重载（uid 变化）",
            beforeUid !== null && afterUid !== null && beforeUid !== afterUid,
            `uid ${beforeUid} → ${afterUid} | ${rl.slice(0, 80)}`,
          );
          // ── 5. 自重载节流（不真自杀：伪造时间戳）──
          writeSelfReloadState(Date.now() - 5000);
          const throttle = await withOpLock(() =>
            reloadPackage("dsh-super-injector"),
          );
          check(
            "自重载节流（<10s 拒绝）",
            throttle.includes("节流"),
            throttle.slice(0, 100),
          );
          ...
          try {
            ...
            if (!libPath) {
              ...
              const lcT = ctx.loader.internal?.loadCache as
                ...
                | undefined;
              for (const u of lcT?.keys() ?? []) {
                if (typeof u !== "string" || !u.endsWith("/lib/index.js"))
                  continue;
                const d = decodeURIComponent(u);
                if (
                  !d.includes("dsh-super-injector") &&
                  !d.includes("cloud-restore")
                )
                  continue;
          ...
          if (libPath) {
            ...
            try {
              ...
              const precheck = await withOpLock(() =>
                reloadPackage("dsh-super-injector"),
              );
              check(
                "预检拦截（拒绝自杀）",
                precheck.includes("预检失败"),
                precheck.slice(0, 150),
              );
          ...
          const patchText = readFileSync(
            join(dirname(profileNodeModules), "cordis.patch.yml"),
            "utf8",
          );
          ...
          check(
            "patch 写入（无 [] 与列表混存）",
            patched && patchValid,
            `bareArray=${hasBareArray} list=${hasList}`,
          );
  ...
  function summarize(results: Array<[string, boolean, string]>): string {
    ...
    const lines = results.map(([n, ok, d]) => {
      ...
      if (ok && detail.startsWith("ERROR:"))
        detail = "[EXPECTED]" + detail.slice(6);
      return `- [${ok ? "PASS" : "FAIL"}] ${n}${detail ? " — " + detail : ""}`;
    });
    auditLog("self-test", `PASS ${pass}/${results.length}`);
    return (
      `===== 注入器自检（PASS ${pass}/${results.length}）=====\n` +
      lines.join("\n")
    );
  }
  ...
  try {
    ...
    logger.warn(
      "[super-injector] systemPrompt.context 重复注册容忍（跳过，新实例继续运行）: %s",
      e instanceof Error ? e.message : String(e),
    );
  }

  logger.info(
    "[super-injector] 就绪：watch %d 目录（%dms），autoRestore=%s",
    watches.length,
    intervalMs,
    String(config.autoRestore),
  );
  ...
  try {
    ...
    if (cmSvc?.pkgMeta && typeof cmSvc.pkgMeta.delete === "function") {
      ...
      if (typeof cmSvc.processOne === "function") {
        ...
        auditLog(
          "client-meta-healed",
          "client-modules pkgMeta 缓存已清并重解析（设置页插件管理 UI 注册）",
        );
      }
    }
  } catch {
  ...
  async function startIngest(dir: string, title?: string): Promise<string> {
    ...
    const agentsSvc = ctx.get("agents") as
      | { create?: (options: any) => Promise<{ agent: { id: string } }> }
      | undefined;
    if (!agentsSvc || typeof agentsSvc.create !== "function")
      return "ERROR: agents 服务不可用（无法新建会话）";
    const sessionId =
      "ingest-" +
    ...
    auditLog(
      "ingest-session",
      `${abs} → 会话 ${handle.agent.id}（${title ?? "内化插件"}）`,
    );
      ...
      ctx.webServer.register({
        ...
        handler: async (req: any, res: any) => {
          ...
          try {
            ...
            const path =
              url.pathname.replace(/^\/super-injector\/api/, "") || "/";
