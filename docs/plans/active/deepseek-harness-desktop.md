...
summary: Active, resumable execution plan for the desktop release, Routing Suite and bundled Skills integration, and progressive Anchored Standard preset — now at 0.1.0-BETA2 / 0.1.1-rc.2.
kind: plan
status: canonical
...
[Confirmed project intent](../../project/intent.md) — `deepseek-harness-code@0.1.0-BETA2`, `@deepseek-ai/dsh@0.1.1-rc.2`, Electron 43.4.0.
...
- [Upstream baseline](../../knowledge/upstream-baseline.md) — pinned versions (0.1.1-rc.2), routing archive digests (SHA-256), 8-plugin inventory, official protocol; verified 2026-08-20.
...
- Harness RC behavior is version-pinned at 0.1.1-rc.2; upgrades require official-source revalidation.
...
- 2026-08-20 — Bumped to `0.1.0-BETA2` / `0.1.1-rc.2` with 8 integrated plugins, immutable routing snapshot, vision-router + better-sidebar service, and global prompt ownership model.
...
- 2026-08-20 — Bumped to `0.1.0-BETA2` / `0.1.1-rc.2`: 8 integrated plugins (ui-polish, updater-check, prompt-principles, vision-router, better-sidebar service, superpowers, code-brand, composition), immutable routing snapshot with SHA-256 pins, global prompt + global `dsh` CLI ownership model, system Node host contract, runtime closure 35 artifacts / 8 critical versions, docs synchronized across workspace and repository.
- 2026-08-20 — Integrated the unfinished `fix/preset-locale-copy` worktree onto 0.1.1-rc.2: four product-owned preset IDs now use locale-aware zh/en client copy with English-only managed fallbacks, while unknown user presets remain metadata-driven. First-launch runtime staging now excludes and self-cleans accidental `.mimosa` hook state that previously surfaced as a misleading Node package installation failure.
- 2026-08-20 — Consolidated all branch/worktree differences on `main`, restored the exact 0.1.1-rc.2 macOS sidebar safe-area patch, made package tests regenerate ignored installer assets, migrated nested agent-named resources to the docs/resource layer, and verified build + 303 unit + 108 Anchored + 24 plugin + 33 package + 2 Playwright tests, `check`, and the 51-artifact runtime closure.
- 2026-08-20 — Diagnosed the installed BETA2 first-launch `ENOENT unlink .../@deepseek-ai__dsh.patch` from live process evidence: one Electron main process had launched two Watchdogs and overlapping runtime installs because a merge retained two lifecycle registration blocks. Consolidated readiness/updater/shutdown under the single lifecycle authority and added a single-flight startup regression before rebuilding the Universal DMG.
- 2026-08-20 — Verified the lifecycle-race repair with 304 unit tests (3 skipped), 108 Anchored, 24 plugin/real-Harness, 33 package, and 2 Playwright tests; `check`, the 51-artifact runtime closure, mounted Universal artifact inspection, and `hdiutil verify` passed. Rebuilt DMG SHA-256: `59c48b8fcf9dd08777a4997ac3064b95ea594dda04c613b4f75eefa7c09c56f4`.
...
- Harness 0.1.1-rc.2 returns 404 for `/api/health` and `/api/`; host health uses child liveness plus the 2xx Web root.
...
- Harness 0.1.1-rc.2 may serve the Web root before Cordis registers `agentPreset.list`; integration readiness now waits on that exact API condition and six consecutive diagnostic repetitions passed.
