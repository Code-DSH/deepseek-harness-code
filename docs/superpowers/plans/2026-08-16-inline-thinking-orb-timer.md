...
**Tech Stack:** React 18, standard React portal objects, `thinking-orbs@0.3.1`, esbuild 0.25.12, exact-version pnpm patching, Vitest/JSDOM, Playwright Chromium.
...
- Consumes: direct status selector `[data-chat-flow] > [role="status"][aria-live="polite"]`, React, the standard `react.portal` object contract, and the desktop plugin `shell.overlay` lifecycle seat.
...
- [x] **Step 3: Add the exact Orb dependency and bounded detector**

Add exact `thinking-orbs@0.3.1` to the desktop plugin and bundle it while keeping React and Harness primitives external. Implement a child-list observer that publishes the direct status anchor and avoids a global query while the current anchor remains connected. Use the standard React portal object contract so the existing Harness renderer handles the portal without bundling a second ReactDOM runtime.

- [x] **Step 4: Implement the inline portal**

Register a lifecycle component through `shell.overlay`; return only a standard React portal object targeting the detected native row. Render:
...
- [x] **Step 5: Prove GREEN and build closure**

Run focused unit, plugin-contract, real-Harness, and browser tests. Rebuild the generated client and assert no unresolved `thinking-orbs` or local controller import remains.
...
- [x] **Step 1: Add RED assertions against the real installed bundle**
...
- [x] **Step 2: Extend the existing exact-version patch**
...
- [x] **Step 3: Verify clean install and behavior**
...
- [x] **Step 1: Restore license and runtime-closure records**
...
- [x] **Step 2: Update architecture and tests**
