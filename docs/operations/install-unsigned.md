...
This is a community distribution and is not an official DeepSeek release. DeepSeek Harness Code `0.1.0-BETA4` is ad-hoc signed, not Apple-notarized. Verify the DMG source and checksum before proceeding.
...
The Universal artifact follows `release/DeepSeek-Harness-Code-<version>-mac-universal.dmg` (for example, `0.1.0-BETA4`). Its checksum is published only after the final package verification succeeds. The same release also publishes a Universal `.zip` consumed by the user-confirmed updater.
