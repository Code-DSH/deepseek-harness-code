...
LAN access is disabled by default. When the user enables **LAN access** in its plugin settings, the separate Electron-owned proxy may listen on `0.0.0.0` while Harness and the desktop renderer stay on loopback. Treat its HTTP URL as trusted-LAN-only: it does not provide Internet exposure or TLS. Use the native **Copy** action and the host's actual LAN IP; `0.0.0.0` is a bind address, not a client destination. The updater plugin exposes read-only `/__dsh/update/status` and `/__dsh/update/events` status routes through the same password/token boundary; remote devices cannot trigger update or restart. Disable the setting to close the listener and invalidate its credentials.

Updates are user-confirmed in the `dsh-updater-check` plugin: the host fetches the allow-listed manifest, publishes live byte progress, verifies SHA-256, and only then enables the final restart action. The update panel has a bounded responsive size; long release notes scroll only in the middle content region while the download/restart actions and progress remain fixed at the bottom. Linux AppImage replacement uses the persistent `$APPIMAGE` path; `.deb` assets are manual; Windows passes the current executable directory to NSIS. A missing platform-native post-update launch check is not treated as a successful release validation.

If an unchanged warm launch unexpectedly reruns every official plugin addition, inspect the startup diagnostics and the reconciliation marker. A complete profile from an older release is adopted once and receives the current marker without rerunning the CLI; a missing or invalid marker only causes reconciliation when the profile links, package identity, link-only bundle state, or managed pnpm store cannot be validated. Any changed or invalid state intentionally reruns the official reconciliation rather than trusting stale state.

If macOS shows no window and a diagnostic stack is stuck in Electron's `SyncProcessRunner::Spawn`, the app has started but its main thread is waiting on the synchronous plugin reconciliation path; this is different from Gatekeeper refusing to launch the app. Quit the stuck BETA2-1 process and install a build containing the reconciliation marker/legacy-profile adoption fix. Do not delete `~/.dsh`, the app user-data directory, or credentials; a complete existing profile is reused on the next launch.
