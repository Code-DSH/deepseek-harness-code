...
Every `BrowserWindow` creation path is registered after Electron's `app.whenReady()` promise resolves, including the macOS `activate` recovery path. Routing Suite startup replaces the profile's empty `[]` patch document with a valid top-level sequence and repairs the exact malformed sequence written by 0.3.0 without duplicating its managed entry or removing valid user entries. The checksum-verified injector and mode-boost entries use paths relative to the app-owned Web profile because the rc.6 Node Loader cannot resolve their profile-local links from bare package specifiers. The packaged 0.3.1 runtime starts this composition successfully and serves the Web root over loopback.
