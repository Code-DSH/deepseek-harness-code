...
The `dsh-updater-check` plugin owns the visible update row and panel. The host publishes replayable status through the fixed `updater` bridge for the desktop window and through authenticated read-only `/__dsh/update/status` JSON plus `/__dsh/update/events` SSE routes for LAN browsers. LAN clients can see availability, byte progress, SHA-256 verification, and ready-to-restart state, but cannot trigger update or restart; those mutations remain host-only. Downloaded assets are not handed to the platform replacement helper until the user selects “Restart and complete update”; the helper then performs the verified replacement and relaunch. The Linux AppImage helper targets the persistent `$APPIMAGE` file rather than Electron's temporary mounted executable, and the Windows NSIS helper passes the running executable's directory as `/D=...`; Debian packages remain notify-only. The downloader rejects manifest, content-range, and streamed sizes above its 512 MiB installer bound before replacement.
