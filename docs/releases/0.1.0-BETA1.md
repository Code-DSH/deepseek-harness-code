...
- **打包与运行**：自包含应用（Chromium、Node、Harness、插件、Watchdog 全部内置），运行时无需全局 Node。
...
| 产物 | SHA-256 |
| --- | --- |
| DeepSeek-Harness-Code-0.1.0-BETA1-mac-universal.dmg（297,288,831 字节） | `7bc3744d786d589a4b64322f00c9f56ce4483e103bc000933b2a6122ccb5f97d` |

> macOS 产物已通过 `scripts/verify-macos-artifact.mjs --universal` 验证：挂载只读校验、运行时闭包、6 个集成插件资产、Anchored Standard 溯源、ad-hoc 签名（`codesign --verify --deep --strict`）与 Universal Mach-O 架构审计全部通过。Windows 安装包的 SHA-256 以 GitHub Release 资产页为准。
...
- Self-contained packaging: Chromium, Node, Harness, plugins, and Watchdog ship inside the app; no global Node required at runtime.
